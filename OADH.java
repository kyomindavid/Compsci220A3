...
public class OADH {
    ...
    private static void execute() throws IOException {
        ...
        while(arraySize != 0) {
            ...
            // ------- h2 ------- 
            String first3DigitsPlus1;
            
            if (finalArray.size() < 3) {
             while (finalArray.size() < 3) {
              finalArray.add(0);
             }
            }

            if (first3DigitsPlus1.length() < 3) {
             first3DigitsPlus1 = first3DigitsPlus1 + 0;
            }
            
            
            first3DigitsPlus1 = "" + (Integer.parseInt("" + finalArray.get(0) + finalArray.get(1) + finalArray.get(2)) + 1);

            if (hashTable.get(key) == 0) {
             hashTable.set(key, 1);
            } else {
             key = (key + Integer.parseInt(first3DigitsPlus1)) % 1000;
